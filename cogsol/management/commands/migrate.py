...
class Command(BaseCommand):
    ...
    def _update_mcp_tool_remote_ids(self, remote_ids: dict[str, Any], payload: Any) -> bool:
        """Extract MCP tool ids from API payloads and store them in ``remote_ids``.

        Accepts either a raw list of tool dicts or dict payloads containing a
        ``results``/``configured_tools``/``tools`` list.
        """
        items: list[Any] = []
        if isinstance(payload, list):
            items = payload
        elif isinstance(payload, dict):
            for key in ("results", "configured_tools", "tools"):
        ...
        for item in items:
            ...
            if not tname:
                continue
            remote_ids.setdefault("mcp_tools", {})[str(tname)] = int(item["id"])
            found = True
        return found
        ...
        *,
        ...
        def _resolve_tool_id(t) -> int | None:
            ...
            for name in candidates:
                ...
                mcp_tool_id = remote_ids.get("mcp_tools", {}).get(name)
                if isinstance(mcp_tool_id, int):
                    return mcp_tool_id
